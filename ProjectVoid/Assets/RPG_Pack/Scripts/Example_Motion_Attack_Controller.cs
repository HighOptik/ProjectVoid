...
public class Example_Motion_Attack_Controller : MonoBehaviour {
	...
	void Start () {
		...
		aux_d = Lifebar.renderer.bounds.size.x;//lifebar width
	...
	void Update () {
		...
		rigidbody2D.velocity = new Vector2(move * maxspeed/2, movey * maxspeed/2);
		//--
		//---
		if(isdead==true){
	...
	void Lifebar_Dead(){
		if(isdead==false){
			if((Lifebar.renderer.bounds.size.x<=0)||(Lifebar.transform.localScale.x<=0)){
				anim.SetBool("dead",true);
				this.gameObject.collider2D.enabled=false;
				this.gameObject.rigidbody2D.isKinematic=true;
				isdead=true;
	...
	void Life_Down(){
		float aux_c = Lifebar.renderer.bounds.size.x;
		...
		auxve.x=auxve.x-(aux_c - Lifebar.renderer.bounds.size.x)/2;
